...
to setup_world
  clear-all
  reset-ticks
  set student_id 2407444
  set student_name "Your Name Here"
...
to run_model
  ...
  ask turtles [
    move_turtle                    ; Move turtles according to rules
    manage_infection               ; Manage infection spread
  ]

  update_global_variables          ; Update global variables in real-time
end

; Turtle movement function
to move_turtle
  ; Set speed and random heading adjustments
  set heading random 360
  fd 0.3                            ; Move forward by 0.3
  ; Implement travel restrictions
  if travel_restrictions [
    if not (pcolor = color) [      ; If not in the correct color region
      set heading towards  (ifelse-value (color = pink) [patch 0 0] [patch 0 0]) ; Move back
    ]
  ]

  ; Implement social distancing
  if social_distancing [
    let nearby (turtles in-radius 1)
    if (any? nearby) [ set heading random 360 ] ; Change heading if too close to another turtle
  ]

  ; Self-isolation
  if (self_isolation and infected_time <= 0) [
    set color blue                   ; Turn blue to indicate self-isolation
                          ; Stop moving
  ]
end

; Manage infection spread function
to manage_infection
  if color = green [
    ; Decrement infected time
    set infected_time infected_time - 1
    if infected_time <= 0 [
      if (random 100 < survival_rate) [  ; Check survival
        set antibodies immunity_duration
        set color black                ; Set color to black for immune
      ]
  ...
  ask turtles with [color = green] [
    ask turtles in-radius 1 [
      if (color = yellow and random 100 < infection_rate) [
        set color green
        set infected_time illness_duration
      ]
    ]
...
to update_global_variables
  ...
  let total_infected count turtles with [color = green]
  set total_infected_percentage (total_infected / total_population) * 100

  let pink_population_count count turtles with [group = "pink turtle"]
  let pink_infected count turtles with [group = "pink turtle" and color = green]
  set pink_infected_percentage (pink_infected / pink_population_count) * 100

  let gray_population_count count turtles with [group = "gray turtle"]
  let gray_infected count turtles with [group = "gray turtle" and color = green]
  set gray_infected_percentage (gray_infected / gray_population_count) * 100

  ; Update death counts
  set total_deaths count turtles with [color = red]
  set pink_deaths count turtles with [group = "pink turtle" and color = red]
  set gray_deaths count turtles with [group = "gray turtle" and color = red]

  ; Update antibodies percentages
  let total_antibodies count turtles with [antibodies > 0]
  set total_antibodies_percentage (total_antibodies / total_population) * 100

  let pink_antibodies count turtles with [group = "pink turtle" and antibodies > 0]
  set pink_antibodies_percentage (pink_antibodies / pink_population_count) * 100

  let gray_antibodies count turtles with [group = "gray turtle" and antibodies > 0]
  set gray_antibodies_percentage (gray_antibodies / gray_population_count) * 100
end
...
"" ""
PENS
"pink polulation" 1.0 0 -2139308 true "" "plot count turtles with [ group = \"pink turtle ]"
...
"" ""
PENS
"Infected" 1.0 0 -13840069 true "" "infected"
"Immune" 1.0 0 -16777216 true "" "Immune"
